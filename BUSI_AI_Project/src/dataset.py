...
def get_development_dataloader(
    config: Dict[str, Any],
    train_mean: Optional[float] = None,
    train_std: Optional[float] = None,
) -> Tuple[DataLoader, pd.DataFrame]:
    """
    Construct DataLoader for the final model trained on all 565 development images.
    Implements a strict pre-training data gate:
      - Loaded development cases == 565
      - Benign cases == 378
      - Malignant cases == 187
      - Test cases accessed == 0
      - Test masks accessed == 0
    Zero validation evaluation: all 565 development images form the training set.
    """
    manifest_path = config.get("data", {}).get("manifest", "data/manifests/BUSI_development_manifest.csv")
    if not os.path.exists(manifest_path):
        alt = os.path.join(os.path.dirname(__file__), "..", manifest_path)
        if os.path.exists(alt):
            manifest_path = alt
        else:
            raise FileNotFoundError(f"Development manifest not found: {manifest_path}")

    dev_df = pd.read_csv(manifest_path)

    # 1. Hard Pre-Training Data Gate Assertions
    total_count = len(dev_df)
    benign_count = int((dev_df["class"] == "benign").sum())
    malignant_count = int((dev_df["class"] == "malignant").sum())

    if total_count != 565:
        raise AssertionError(f"[DATA GATE FAILURE] Expected exactly 565 development cases, got {total_count}")
    if benign_count != 378:
        raise AssertionError(f"[DATA GATE FAILURE] Expected exactly 378 benign cases, got {benign_count}")
    if malignant_count != 187:
        raise AssertionError(f"[DATA GATE FAILURE] Expected exactly 187 malignant cases, got {malignant_count}")

    # 2. Test Set Isolation Verification (Zero Test Leakage)
    ...
    if not os.path.exists(test_manifest_path):
        alt_test = os.path.join(os.path.dirname(__file__), "..", test_manifest_path)
        if os.path.exists(alt_test):
            test_manifest_path = alt_test
    ...
    if os.path.exists(test_manifest_path):
        ...
        overlap_imgs = dev_imgs.intersection(test_imgs)
        if len(overlap_imgs) > 0:
            raise AssertionError(f"[DATA GATE FAILURE] Test image leakage detected! Overlap: {overlap_imgs}")

        dev_masks = set(dev_df["mask_path"].apply(os.path.normpath))
        test_masks = set(test_df["mask_path"].apply(os.path.normpath))
        overlap_masks = dev_masks.intersection(test_masks)
        if len(overlap_masks) > 0:
            raise AssertionError(f"[DATA GATE FAILURE] Test mask leakage detected! Overlap: {overlap_masks}")

    batch_size = config.get("training", {}).get("batch_size", 8)
    num_workers = config.get("hardware", {}).get("num_workers", 0)
    raw_root = config.get("data", {}).get("raw_root", "")

    # Training transforms apply to all 565 development images
    train_transform = build_transforms(config, is_train=True, train_mean=train_mean, train_std=train_std)
    dev_dataset = BUSIDataset(dev_df, transform=train_transform, raw_root=raw_root)
    ...
    print(f"[DATA GATE PASS] Development DataLoader initialized with {total_count} images ({benign_count} benign, {malignant_count} malignant). Zero test leakage verified.")
    return dev_loader, dev_df
