...
def parse_args():
    parser = argparse.ArgumentParser(description="Train baseline Plain U-Net using 3-Fold CV or Final Model on BUSI dataset.")
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        "--train_final",
        action="store_true",
        help="Train the final Plain U-Net model on all 565 development images for a fixed epoch budget (default: 86 epochs)."
    )
    parser.add_argument(
        "--final_epochs",
        type=int,
        default=86,
        help="Fixed epoch budget for final model training on all 565 development images (default: 86)."
    )
...
def train_single_fold(
    ...
    base_chk_dir = config.get("outputs", {}).get("checkpoint_dir", "outputs/checkpoints/exp_seg_02")
    base_log_dir = config.get("outputs", {}).get("log_dir", "outputs/logs/exp_seg_02")
    base_rep_dir = config.get("outputs", {}).get("report_dir", "outputs/reports/exp_seg_02")
...
def plot_final_training_curves(history: Dict[str, List], output_path: str):
    """Generate and save training loss and learning rate curves for the final model."""
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    epochs = range(1, len(history["epoch"]) + 1)
    ...
    # Loss Curve
    axes[0].plot(epochs, history["train_loss"], label="Train Loss (BCE + Dice)", color="#1f77b4", linewidth=2)
    final_loss = history["train_loss"][-1]
    axes[0].scatter([len(history["epoch"])], [final_loss], color="red", s=80, zorder=5, label=f"Final Loss: {final_loss:.4f} (Ep {len(history['epoch'])})")
    axes[0].set_title("Final Model Training Loss (N=565 Development Images)", fontsize=12)
    axes[0].set_xlabel("Epoch")
    axes[0].set_ylabel("Loss")
    axes[0].legend()
    axes[0].grid(True, linestyle="--", alpha=0.6)
    ...
    # Learning Rate Curve
    axes[1].plot(epochs, history["learning_rate"], label="Cosine LR Schedule", color="#ff7f0e", linewidth=2)
    axes[1].set_title(f"Learning Rate vs Epoch (T_max={len(history['epoch'])})", fontsize=12)
    axes[1].set_xlabel("Epoch")
...
def train_final_model(
    ...
    """
    Train the final Plain U-Net model on all 565 development images for exactly final_epochs (86).
    Enforces strict Quality Gate:
      - 565 development images (378 benign, 187 malignant)
      - Zero test set access (63 locked test images isolated)
      - Zero validation evaluation (all 565 images used for training)
      - No early stopping (completes all 86 epochs)
    """
    if device is None:
    ...
    print("=" * 80)
    print("STARTING FINAL MODEL TRAINING: EXP-SEG-02 PLAIN U-NET BASELINE")
    print(f"Fixed Epoch Budget: {final_epochs} epochs (Median of CV Folds [70, 86, 100])")
    print(f"Device: {device} | Seed: {seed}")
    print("=" * 80)

    # 1. Hard Pre-Training Data Gate & Development DataLoader
    dev_loader, dev_df = get_development_dataloader(config)

    total_count = len(dev_df)
    benign_count = int((dev_df["class"] == "benign").sum())
    malignant_count = int((dev_df["class"] == "malignant").sum())

    print(f"[INFO] Final Training on ALL {total_count} development images ({len(dev_loader)} batches, batch_size={config.get('training', {}).get('batch_size', 8)})")
    print(f"[INFO] Class Distribution: {benign_count} Benign ({benign_count/total_count*100:.2f}%), {malignant_count} Malignant ({malignant_count/total_count*100:.2f}%)")
    print("[INFO] Test Set Status: 63 LOCKED TEST IMAGES UNTOUCHED & ZERO ACCESS VERIFIED.")
    ...
    lr = float(loss_cfg.get("lr", 1e-3))
    batch_size = int(loss_cfg.get("batch_size", 8))

    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=final_epochs, eta_min=1e-6)

    # 3. Output Directories
    base_chk_dir = os.path.join(config.get("outputs", {}).get("checkpoint_dir", "outputs/checkpoints/exp_seg_02"), "final_model")
    base_log_dir = os.path.join(config.get("outputs", {}).get("log_dir", "outputs/logs/exp_seg_02"), "final_model")
    base_rep_dir = os.path.join(config.get("outputs", {}).get("report_dir", "outputs/reports/exp_seg_02"), "final_model")

    os.makedirs(base_chk_dir, exist_ok=True)
    os.makedirs(base_log_dir, exist_ok=True)
    os.makedirs(base_rep_dir, exist_ok=True)

    final_checkpoint_path = os.path.join(base_chk_dir, "final_model.pt")
    history_csv_path = os.path.join(base_log_dir, "training_history.csv")
    curves_png_path = os.path.join(base_rep_dir, "training_curves.png")
    metadata_json_path = os.path.join(base_rep_dir, "training_metadata.json")
    ...
    for epoch in range(1, final_epochs + 1):
        ...
        train_loss = train_one_epoch(model, dev_loader, optimizer, criterion, device)
        scheduler.step()
        ...
        history["elapsed_seconds"].append(epoch_time)
        history["cumulative_elapsed_seconds"].append(cumulative_time)

        print(
            f"Final Model | Ep [{epoch:03d}/{final_epochs:03d}] | "
            f"Train Loss: {train_loss:.4f} | "
            f"LR: {current_lr:.2e} | "
            f"Time: {epoch_time:.2f}s | "
            f"Cum Time: {cumulative_time:.2f}s",
            flush=True
        )
    ...
    print(f"[INFO] Final training history saved to: {history_csv_path}")

    # 5. Generate & Save Training Curves
    plot_final_training_curves(history, curves_png_path)
    print(f"[INFO] Final training curves saved to: {curves_png_path}")

    # 6. Collect System Metadata & Config
    sys_meta = get_system_metadata(device=device)

    final_metadata = {
        "experiment": "EXP-SEG-02",
        "model_designation": "Final Baseline Plain U-Net Model (Trained on 100% Development Set)",
        "dataset": {
            "manifest": config.get("data", {}).get("manifest", "data/manifests/BUSI_development_manifest.csv"),
            "total_development_images": total_count,
            "benign_images": benign_count,
            "malignant_images": malignant_count,
            "malignant_percentage": round((malignant_count / total_count) * 100, 2),
        ...
        "input_configuration": {
            ...
            "grayscale": config.get("preprocessing", {}).get("grayscale", True),
            "normalize": config.get("preprocessing", {}).get("normalize", "minmax"),
            "augmentation": config.get("preprocessing", {}).get("augmentation", True),
        },
        "system_environment": sys_meta,
        "training_results": {
            "final_epoch": final_epochs,
            "final_train_loss": float(history["train_loss"][-1]),
            "total_duration_seconds": float(total_training_duration),
    ...
    print(f"[INFO] Final training metadata saved to: {metadata_json_path}")
    ...
    print(f"[INFO] Final model checkpoint saved to: {final_checkpoint_path}")

    mins, secs = divmod(total_training_duration, 60)
    print("\n" + "=" * 80)
    print("FINAL MODEL TRAINING COMPLETE")
    print(f"Total Duration: {int(mins)}m {secs:.2f}s ({final_epochs} epochs)")
...
def main():
    ...
    if args.train_final:
        train_final_model(config=config, final_epochs=args.final_epochs, device=device)
        return

    print("=" * 80)
    print("STARTING STRATIFIED 3-FOLD CV EXPERIMENT: EXP-SEG-02 (Plain U-Net Baseline)")
    ...
    # Combine out-of-fold validation predictions (N=565)
    ...
    # Compute overall pooled metrics across all 565 development out-of-fold predictions
    ...
    cv_summary_data = {
        ...
        "protocol": "3-Fold Stratified Development Cross-Validation (N=565)",
    ...
    with open(md_report_path, "w", encoding="utf-8") as f:
        f.write("# EXP-SEG-02 Plain U-Net: Development 3-Fold Cross-Validation Report\n\n")
        f.write("## Overview & Protocol\n")
        f.write("- **Model**: Plain U-Net (Depth=4, BaseChannels=64)\n")
        f.write("- **Evaluation Protocol**: 3-Fold Stratified Development Cross-Validation\n")
        f.write(f"- **Total Development Images Evaluated**: {len(combined_val_df)} (378 benign, 187 malignant)\n")
        f.write(f"- **Total Training Duration**: {total_cv_duration / 60:.2f} minutes\n\n")
        f.write("## Primary Out-of-Fold Development Performance\n\n")
